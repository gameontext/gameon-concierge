...
/**
 * The dynamic strategy will make it harder to draw out a map of the rooms
...
public class TestManualWiringConcierge {
	...
	public void registerARoom() {
		...
		assertEquals("The start room should be the first room we add", "Starting Room",
				c.getStartingRoom().getRoomId());
	}
	...
	public void placeTwoRoomsNearEachother() {
		...
		assertEquals("The secondRoom should be returned", secondRoomAsRegistered.getRoomName(),
				secondRoomAsFound.getRoomId());
	}
	...
	public void MoveToNewRoomAndBack() {
		...
		secondRoomExits
				.add(new Exit("South", "Starting Room", "A door between the second room back to the starting room"));
