...
@WebFilter(filterName = "registrationAuthFilter", urlPatterns = { "/*" })
public class ConciergeAuthFilter implements Filter {
	private static final String CHAR_SET = "UTF-8";
	private static final String HMAC_ALGORITHM = "HmacSHA256";
	
	/** timeout for requests, default to 5 seconds */
	...
	/**
	 * Used API Key (Eg, one that has been seen, and is being tracked to ensure
	 * no reuse) Equality / Hashcode is determined by apikey string alone. Sort
	 * order is provided by key timestamp.
	 */
	...
	/** keys already received, prevent replay attacks */
	private static final Set<UsedKey> usedKeys = Collections.synchronizedSet(new LinkedHashSet<UsedKey>()); 
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// we're a single filter, but we protect different paths with different
		// keys.
		...
		while (!state.equals(AuthenticationState.PASSED)) {
			switch (state) {
			case hasQueryString: 
				// check that there is a query string which
				// will contain the service ID and api key
			...
			case hasAPIKeyParam: // check there is an apikey parameter
				...
				break;
			case isAPIKeyValid: 
				// validate API key against all parameters
			...
			case hasKeyExpired: // check that key has not timed out
				...
				break;
			case checkReplay: 
				// simple replay check - only allows the one time
				// use of API keys, storing time allows expired
				// keys to be purged
				boolean alreadyPresent = usedKeys.add(new UsedKey(apikey, time));
				// the set of keys is sorted with oldest (smallest) timestamp
				// first so we can iterate from the oldest key,
